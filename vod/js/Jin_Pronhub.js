...
async function getClassList(args) {
    let listUrl = appConfig.webSite + '/categories'
    ...
    try {
        ...
        if (proData) {
            ...
            let allClass = document.querySelectorAll('#categoriesListingWrapper')[1].querySelectorAll('a.js-flipbookOn.js-videoPreview.webm-videoPreview')
            let list = []
            for (let i = 0; i < allClass.length; i++) {
                const item = allClass[i]
                let videoClass = new VideoClass()
                videoClass.type_id = item.getAttribute('href')
                videoClass.type_name = item.getAttribute('alt')
...
async function getVideoList(args) {
    ...
    listUrl = listUrl.includes('/categories') ||
        listUrl.includes('/vr') ||
        listUrl.includes('/hd') ||
        listUrl.includes('/sfw') ||
        listUrl.includes('/popularwithwomen') ||
        listUrl.includes('/interactive') ||
        listUrl.includes('/transgender')

        ? listUrl + '?page=' + args.page : listUrl + '&page=' + args.page
    try {
        let pro = await req(listUrl, { headers: appConfig.headers })
        backData.error = pro.error
        let proData = pro.data
        if (proData) {
            ...
            let allVideo = document.querySelectorAll("li.pcVideoListItem.js-pop.videoblock.videoBox.withKebabMenu a.fade.videoPreviewBg.linkVideoThumb.js-linkVideoThumb.img.fadeUp")

            if (allVideo.length === 0) {
                throw new Error('未找到视频元素' + allVideo + ' ' + listUrl)
            }
            let videos = []
            for (let index = 0; index < allVideo.length; index++) {
                const element = allVideo[index]
                let vodUrl = element?.attributes['href'] ?? ''
